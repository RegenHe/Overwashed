...
- `ClientKitchenFlowControllerBase.RoundTimer` publicly exposes the active `IClientRoundTimer`, and `ClientFlowControllerBase.InRound` identifies the round lifecycle.
- `ClientRoundTimer.TimeElapsed` counts upward from zero while `KitchenLevelConfigBase.GetTimeLimit()` supplies the normal round limit; remaining time is their difference.
